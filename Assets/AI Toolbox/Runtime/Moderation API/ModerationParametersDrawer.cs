...
namespace AiToolbox {
[CustomPropertyDrawer(typeof(ModerationParameters))]
public class ModerationParametersDrawer : PropertyDrawer {
    private float _height;

    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        label.text = "AI Moderation Parameters";
        label.tooltip = "Settings for the backend of content moderation requests.";
        ...
        // Question mark button
        {
            ...
            if (GUI.Button(buttonRect, icon, EditorStyles.label)) {
                Application.OpenURL("https://ai-toolbox.dustyroom.com/moderation");
            }
        }
        ...
        if (property.isExpanded) {
            ...
            // API Key
            {
                var apiKeyProperty = property.FindPropertyRelative(nameof(ModerationParameters.apiKey));
                ...
                _height += rect.height + EditorGUIUtility.standardVerticalSpacing;
                var encryptionProperty = property.FindPropertyRelative(nameof(ModerationParameters.apiKeyEncryption));
                ...
                var passwordProperty =
                    property.FindPropertyRelative(nameof(ModerationParameters.apiKeyEncryptionPassword));
            ...
            // API Key encryption
            {
                ...
                if (encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.None &&
                    !string.IsNullOrEmpty(property.FindPropertyRelative(nameof(ModerationParameters.apiKey))
                                              .stringValue)) {
                ...
                if (showRemoteConfigField) {
                    var remoteConfigKeyProperty =
                        property.FindPropertyRelative(nameof(ModerationParameters.apiKeyRemoteConfigKey));
            ...
            // Model
            {
                var modelProperty = property.FindPropertyRelative(nameof(ModerationParameters.moderationModel));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
                var modelLabel = new GUIContent("Model", "The AI moderation model to use.");
            ...
            // Timeout
            {
                var timeoutProperty = property.FindPropertyRelative(nameof(ModerationParameters.timeout));
            ...
            // Throttle
            {
                var throttleProperty = property.FindPropertyRelative(nameof(ModerationParameters.throttle));
