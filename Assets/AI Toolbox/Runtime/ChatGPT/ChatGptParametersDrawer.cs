...
namespace AiToolbox {
[CustomPropertyDrawer(typeof(ChatGptParameters))]
public class ChatGptParametersDrawer : PropertyDrawer {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        ...
        if (property.isExpanded) {
            ...
            // API Key
            {
                var apiKeyProperty = property.FindPropertyRelative(nameof(ChatGptParameters.apiKey));
                ...
                _height += rect.height + EditorGUIUtility.standardVerticalSpacing;
                var encryptionProperty = property.FindPropertyRelative(nameof(ChatGptParameters.apiKeyEncryption));
                ...
                var passwordProperty =
                    property.FindPropertyRelative(nameof(ChatGptParameters.apiKeyEncryptionPassword));
            ...
            // API Key encryption
            {
                ...
                if (encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.None &&
                    !string.IsNullOrEmpty(property.FindPropertyRelative(nameof(ChatGptParameters.apiKey))
                                              .stringValue)) {
                ...
                if (showRemoteConfigField) {
                    var remoteConfigKeyProperty =
                        property.FindPropertyRelative(nameof(ChatGptParameters.apiKeyRemoteConfigKey));
            ...
            // Role
            {
                var roleProperty = property.FindPropertyRelative(nameof(ChatGptParameters.role));
            ...
            // Timeout
            {
                var timeoutProperty = property.FindPropertyRelative(nameof(ChatGptParameters.timeout));
            ...
            // Throttle
            {
                var throttleProperty = property.FindPropertyRelative(nameof(ChatGptParameters.throttle));
