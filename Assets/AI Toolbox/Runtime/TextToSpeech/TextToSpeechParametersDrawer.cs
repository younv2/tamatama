...
namespace AiToolbox {
[CustomPropertyDrawer(typeof(TextToSpeechParameters))]
public class TextToSpeechParametersDrawer : PropertyDrawer {
    private float _height;

    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        label.text = "Text to Speech Parameters";
        label.tooltip = "Settings for the backend of the OpenAI Text to Speech API.";
        ...
        // Question mark button
        {
            ...
            if (GUI.Button(buttonRect, icon, EditorStyles.label)) {
                Application.OpenURL("https://ai-toolbox.dustyroom.com/text-to-speech");
            }
        }
        ...
        if (property.isExpanded) {
            ...
            // API Key
            {
                var apiKeyProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.apiKey));
                ...
                _height += rect.height + EditorGUIUtility.standardVerticalSpacing;
                var encryptionProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.apiKeyEncryption));
                ...
                var passwordProperty =
                    property.FindPropertyRelative(nameof(TextToSpeechParameters.apiKeyEncryptionPassword));
            ...
            // API Key encryption
            {
                ...
                if (encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.None &&
                    !string.IsNullOrEmpty(property.FindPropertyRelative(nameof(TextToSpeechParameters.apiKey))
                                              .stringValue)) {
                ...
                if (showRemoteConfigField) {
                    var remoteConfigKeyProperty =
                        property.FindPropertyRelative(nameof(TextToSpeechParameters.apiKeyRemoteConfigKey));
            ...
            // Model
            {
                var modelProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.model));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
            ...
            // Voice
            {
                var voiceProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.voice));
                var rect = new Rect(position.x, position.y + _height, width - 20, EditorGUIUtility.singleLineHeight);
                EditorGUI.PropertyField(rect, voiceProperty);
                _height += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;
                ...
                // Question mark button
                {
                    ...
                    EditorGUIUtility.AddCursorRect(buttonRect, MouseCursor.Link);
                    var icon = EditorGUIUtility.IconContent("_Help");
                    icon.tooltip = "Open previews of the voices";
                    if (GUI.Button(buttonRect, icon, EditorStyles.label)) {
                        Application.OpenURL("https://platform.openai.com/docs/guides/text-to-speech/voice-options");
                    }
                }
            }
            ...
            // Speed
            {
                var speedProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.speed));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
                EditorGUI.PropertyField(rect, speedProperty);
                _height += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;
            }
            ...
            // Timeout
            {
                var timeoutProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.timeout));
            ...
            // Throttle
            {
                var throttleProperty = property.FindPropertyRelative(nameof(TextToSpeechParameters.throttle));
